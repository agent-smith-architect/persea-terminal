...
func TestRotationOrdinaryPredecessorOutputDoesNotSpendR1WhileHeld(t *testing.T) {
	...
	defer realm.Close()
	predecessor := rotationKey("$rotation-ordinary-held", 1)
	...
	if pane.rotationRollbackLogical != RotationPendingCapBytes || pane.rotationRollbackRecords != RotationPendingCapRecords {
		t.Fatalf("ordinary pre-boundary output spent R1: bytes=%d/%d records=%d/%d", pane.rotationRollbackLogical, RotationPendingCapBytes, pane.rotationRollbackRecords, RotationPendingCapRecords)
	}
	if _, err := realm.Append(predecessor, []byte("17")); err != nil {
		t.Fatalf("17th ordinary record refused with ordinary room available: %v (live=%v)", err, realm.UnifiedEligible(predecessor))
	}
}
...
func TestRotationArmedCreditRecordExhaustionDoesNotBlockOrdinaryOutput(t *testing.T) {
	options := rotationOptions(t)
	realm, err := openRealm(options, realJournalOps())
	if err != nil {
		t.Fatal(err)
	...
	defer realm.Close()
	predecessor := rotationKey("$rotation-armed", 1)
	successor := rotationKey("$rotation-armed", 2)
	admitRotationPredecessor(t, realm, predecessor)
	capacity, err := realm.BeginRotationCapacity(predecessor)
	if err != nil {
		t.Fatal(err)
	...
	if _, err := realm.Append(predecessor, []byte("ordinary")); err != nil {
		t.Fatalf("ordinary output refused after the armed credit's records ran out: %v (live=%v)", err, realm.UnifiedEligible(predecessor))
	}
	capacity.Release()
...
func TestRotationRecordCapNeverTouchesUnrelatedPanes(t *testing.T) {
	options := rotationOptions(t)
	...
	defer realm.Close()
	predecessor := rotationKey("$rotation-unrelated", 1)
	other := rotationKey("$rotation-unrelated-other", 1)
	admitRotationPredecessor(t, realm, predecessor)
...
func TestRotationCapacityCannotMaterializeTwice(t *testing.T) {
	options := rotationOptions(t)
	realm, err := openRealm(options, realJournalOps())
	if err != nil {
		t.Fatal(err)
	...
	defer realm.Close()
	predecessor := rotationKey("$rotation-twice", 1)
	admitRotationPredecessor(t, realm, predecessor)
	...
	first, err := realm.BeginRotatedPane(rotationKey("$rotation-twice", 2), geometry, capacity)
	if err != nil {
		t.Fatal(err)
	}
	slotsBefore, _, _ := rotationLedger(realm)
	second, err := realm.BeginRotatedPane(rotationKey("$rotation-twice", 3), geometry, capacity)
	if err == nil || second != nil {
		slots, _, _ := rotationLedger(realm)
		live := 0
		...
		t.Fatalf("consumed capacity materialized a second successor: err=%v slots=%d (before %d) live-generations=%d", err, slots, slotsBefore, live)
	}
	first.Commit()
}
