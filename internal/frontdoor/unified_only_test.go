...
func TestTerminalRequiresUnifiedBeforeCapabilityConsumption(t *testing.T) {
	for _, engine := range []string{"", ", persea-engine.", ", persea-engine.other"} {
		for _, mode := range []string{"observe", "control"} {
			...
			r.Header.Set("Sec-WebSocket-Protocol", "persea-terminal.v2, persea-handle."+handle+", persea-mode."+mode+", persea-csrf."+testCSRF+", persea-history.5000"+engine)
...
func currentFixtureDial(addr, handle, mode string) (*websocket.Conn, *http.Response, error) {
	dialer := websocket.Dialer{Subprotocols: []string{"persea-engine.unified-dev", "persea-terminal.v2", "persea-handle." + handle, "persea-mode." + mode, "persea-csrf." + testCSRF, "persea-history.5000"}}
