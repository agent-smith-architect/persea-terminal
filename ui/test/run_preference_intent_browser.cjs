...
// See test/preference_intent_browser_harness.ts for what each case proves.
...
async function main() {
  childProcess.execFileSync(path.join(UI, "node_modules/.bin/esbuild"), [
    path.join(UI, "test/preference_intent_browser_harness.ts"),
    "--bundle", "--platform=browser", "--format=iife",
    `--outfile=${path.join(UI, "dist/test/preference_intent_browser_harness.js")}`,
  ], { stdio: "inherit" });

  const html = `<!doctype html>
<html><head><meta charset="utf-8"><link rel="stylesheet" href="/xterm.css"><link rel="stylesheet" href="/attachment.css">
<style>html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#111318}.preference-intent-root{width:100%;height:100%}</style>
</head><body><script src="/preference_intent_harness.js"></script></body></html>`;
  const server = http.createServer((request, response) => {
    ...
    const routes = {
      "/preference_intent_harness.js": path.join(UI, "dist/test/preference_intent_browser_harness.js"),
  ...
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), "persea-preference-intent-"));
  ...
  try {
    ...
    for (let attempt = 0; attempt < 200; attempt++) {
      const value = await cdp.send("Runtime.evaluate", {
        expression: 'document.body && document.body.dataset.preferenceIntentHarnessReady === "true"',
        returnByValue: true,
      });
      if (value.result.value === true) { ready = true; break; }
      await delay(25);
    }
    assert(ready, "the preference intent harness never signalled ready");
    outcome = await evaluate(cdp, "window.__preferenceIntentHarness.run()");
  } finally {
    cdp?.close();
    server.close();
    if (!chromeExit) {
  ...
  const artifact = process.env.PERSEA_PREFERENCE_INTENT_ARTIFACT;
