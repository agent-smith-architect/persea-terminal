...
const ENGINE = process.env.PERSEA_LOADING_STATE_ENGINE || "chromium";
const MODULE = process.env.PERSEA_PLAYWRIGHT_MODULE || require.resolve("playwright");
const EVIDENCE = path.resolve(process.env.PERSEA_LOADING_STATE_EVIDENCE_DIR || path.join("/tmp", `persea-loading_state-${ENGINE}`));
...
async function main() {
  ...
  try {
    ...
    for (const scenario of [{ count: 1, viewport: { width: 1100, height: 760 }, label: "single-desktop" }, { count: 6, viewport: { width: 1280, height: 800 }, label: "workspace-six" }, { count: 1, viewport: { width: 390, height: 844 }, label: "single-phone" }]) {
      ...
      const mounted = await page.evaluate((count) => window.__loading_state.reset(count), scenario.count);
      await page.evaluate(() => new Promise((resolve) => requestAnimationFrame(() => resolve(undefined))));
      const mountToFirstFrameMs = Date.now() - mountStart;
      const firstFrame = await page.evaluate(() => window.__loading_state.snapshot());
      ...
      const prepared = await page.evaluate(() => window.__loading_state.prepareAll());
      await page.waitForTimeout(260);
      const delayed = await page.evaluate(() => window.__loading_state.snapshot());
      ...
      const committed = await page.evaluate(() => window.__loading_state.commitAll());
      ...
      assert(committed.after.panes.every((pane) => !pane.loadingVisible && pane.rendered.includes(`LOADING_STATE-REPLAY-${pane.name}`)), `${scenario.label}: loader not removed synchronously with committed replay paint: ${JSON.stringify(committed.after.panes)}`);
    ...
    const failed = await page.evaluate(() => window.__loading_state.failAll("unified_unavailable"));
    ...
    await page.evaluate(() => window.__loading_state.commitAll());
    await page.waitForFunction(() => window.__loading_state.snapshot().panes[0]?.rendered.includes("LOADING_STATE-REPLAY"));
    const exhausted = await page.evaluate(() => window.__loading_state.exhaustAll());
    assert(exhausted.panes[0].reconnectVisible && exhausted.panes[0].rendered.includes("LOADING_STATE-REPLAY"), `retry exhaustion must retain output and offer Reconnect: ${JSON.stringify(exhausted)}`);
    await page.evaluate(() => document.querySelector(".persea-unified-notice__reconnect").click());
    assert((await page.evaluate(() => window.__loading_state.snapshot())).reconnectRequests === 0, "synthetic reconnect bypassed trusted activation");
    await page.getByRole("button", { name: "Reconnect", exact: true }).click();
    const reconnecting = await page.evaluate(() => window.__loading_state.snapshot());
    ...
    for (const reason of ["session_gone", "identity_ambiguous", "source_binding_unavailable", "bad_liveness", "reconnect_unavailable"]) {
      const refused = await page.evaluate((reason) => window.__loading_state.exhaustAll(reason), reason);
